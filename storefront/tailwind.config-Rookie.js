...
// Attempt to resolve Medusa UI package paths so Tailwind can scan its components.
// In environments where the dependency is absent (e.g. docs site builds) we skip it gracefully.
let uiContentPaths = []
try {
  const medusaUiRoot = path.dirname(require.resolve("@medusajs/ui/package.json"))
  uiContentPaths = [
    path.join(medusaUiRoot, "**/*.{js,jsx,ts,tsx}"),
  ]
} catch (err) {
  if (process.env.NODE_ENV === "development") {
    console.warn("@medusajs/ui not found; skipping Tailwind content scan for that package.")
  }
}

const localUiRoot = path.join(__dirname, "packages", "ui", "src")
if (fs.existsSync(localUiRoot)) {
  uiContentPaths.push(path.join(localUiRoot, "**/*.{js,jsx,ts,tsx}"))
}
