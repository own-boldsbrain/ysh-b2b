...
```mermaid
graph LR
    A[Cliente solicita cotação] --> B{Aplicação?}
    B -->|Residencial| C[Quick Reference - Residencial]
    B -->|Comercial| D[Quick Reference - Comercial]
    B -->|Industrial| E[Quick Reference - Industrial]
    C --> F[Consolidado - Specs Detalhadas]
    D --> F
    E --> F
    F --> G[Cotação Final]
```
...
```mermaid
graph LR
    A[Requisitos Cliente] --> B[Consolidado - Tecnologias]
    B --> C[Comparativo de Performance]
    C --> D[Seleção Fabricante]
    D --> E[Quick Reference - Cálculos]
    E --> F[Projeto Técnico]
```
