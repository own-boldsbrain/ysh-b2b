...
class AdvancedImageMapper:
    ...
    def analyze_huawei(self) -> MappingResult:
        """Analyze Huawei HTML files for image URLs"""

        print("\n[HUAWEI] Analyzing HTML files for image URLs...")

        scrapers_dir = self.base_path / "scrapers-inverters"
        huawei_files = [
            (
                "Lista de Produtos Residenciais ｜ HUAWEI Smart PV Brasil "
                "(21_10_2025 02：59：45).html"
            ),
            (
                "Lista de Produtos Comerciais e Industriais ｜ "
                "HUAWEI Smart PV Brasil (21_10_2025 03：01：30).html"
            ),
        ]

        result = MappingResult(
            distributor="huawei",
            total_products=0,  # We'll count unique images
            with_real_images=0,
            with_placeholder=0,
            urls_to_download=[],
            mapped_images=0,
        ...
        for filename in huawei_files:
            ...
            try:
                ...
                for img in img_tags:
                    src = img.get("src")
                    if src:
                        full_url = urljoin("https://solar.huawei.com", src)
                        if full_url not in image_urls:
                            image_urls.add(full_url)
                            result.urls_to_download.append(full_url)

            except Exception as e:
                result.errors.append(f"Error processing {filename}: {e}")

        result.with_real_images = len(image_urls)
        result.total_products = len(image_urls)  # Approximate

        print(f"[HUAWEI] Found {len(image_urls)} unique image URLs")

        # Save URLs
        urls_file = (
            self.base_path / "distributors" / "huawei" / "huawei_image_urls.json"
        )
        urls_file.parent.mkdir(exist_ok=True)
        with open(urls_file, "w", encoding="utf-8") as f:
            json.dump(list(image_urls), f, ensure_ascii=False, indent=2)

        print(f"[HUAWEI] ✅ Saved URLs to {urls_file.name}")

        return result

    def analyze_deye(self) -> MappingResult:
        """Analyze Deye HTML files for image URLs"""

        print("\n[DEYE] Analyzing HTML files for image URLs...")

        scrapers_dir = self.base_path / "scrapers-inverters"
        deye_files = [
            (
                "Manual do produto- NingBo Deye Inverter Technology Co.,Ltd "
                "(21_10_2025 03：00：43).html"
            ),
            (
                "Sistemas de armazenamento de energia residencial- "
                "NingBo Deye Inverter Technology Co.,Ltd "
                "(21_10_2025 02：54：10).html"
            ),
        ]
        ...
        for filename in deye_files:
            ...
            try:
                ...
                for img in img_tags:
                    src = img.get("src")
                    if src:
                        full_url = urljoin("https://pt.deyeinverter.com", src)
                        if full_url not in image_urls:
                            image_urls.add(full_url)
                            result.urls_to_download.append(full_url)
        ...
        result.with_real_images = len(image_urls)
        result.total_products = len(image_urls)

        print(f"[DEYE] Found {len(image_urls)} unique image URLs")

        # Save URLs
        urls_file = self.base_path / "distributors" / "deye" / "deye_image_urls.json"
        urls_file.parent.mkdir(exist_ok=True)
        with open(urls_file, "w", encoding="utf-8") as f:
            json.dump(list(image_urls), f, ensure_ascii=False, indent=2)

        print(f"[DEYE] ✅ Saved URLs to {urls_file.name}")
    ...
    def generate_report(
        ...
        huawei_result: MappingResult,
        deye_result: MappingResult,
    ) -> str:
        ...
        # Huawei
        report.append("📦 HUAWEI")
        report.append(f"   Total Images Found: {huawei_result.with_real_images}")
        report.append(f"   URLs to Download: " f"{len(huawei_result.urls_to_download)}")

        if huawei_result.errors:
            report.append(f"   Errors: {len(huawei_result.errors)}")

        report.append("")

        # Deye
        report.append("📦 DEYE")
        report.append(f"   Total Images Found: {deye_result.with_real_images}")
        report.append(f"   URLs to Download: " f"{len(deye_result.urls_to_download)}")

        if deye_result.errors:
            report.append(f"   Errors: {len(deye_result.errors)}")
...
def main():
    ...
    # Analyze Huawei
    huawei_result = mapper.analyze_huawei()

    # Analyze Deye
    deye_result = mapper.analyze_deye()

    # Extract FOTUS CSV data
    mapper.extract_fotus_csv_data()
    ...
    report = mapper.generate_report(
        neosolar_result, fotus_result, huawei_result, deye_result
    )
    print(report)
