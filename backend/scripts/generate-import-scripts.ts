/**
 * Gera scripts de import para cada distribuidor baseado no template Fortlev
 * Uso: npx tsx scripts/generate-import-scripts.ts
 * 
 * Cria:
 * - scripts/import-{distributor}-to-db.ts para cada um dos 7 distribuidores
 * - Pronto para usar: npx tsx scripts/import-solfacil-to-db.ts, etc.
 */
...
const distributors = [
  {
    id: 'fortlev',
    name: 'Fortlev',
    catalogFile: 'fortlev-catalog-full.json',
  },
  {
    id: 'solfacil',
    name: 'Solfácil',
    catalogFile: 'solfacil-catalog-full.json',
  },
  {
    id: 'fotus',
    name: 'Fotus',
    catalogFile: 'fotus-catalog-full.json',
  },
  {
    id: 'neosolar',
    name: 'Neosolar',
    catalogFile: 'neosolar-catalog-full.json',
  },
  {
    id: 'odex',
    name: 'Odex',
    catalogFile: 'odex-catalog-full.json',
  },
  {
    id: 'edeltec',
    name: 'Edeltec',
    catalogFile: 'edeltec-catalog-full.json',
  },
  {
    id: 'dynamis',
    name: 'Dynamis',
    catalogFile: 'dynamis-catalog-full.json',
  },
];

function generateImportScript(distributor: typeof distributors[0]): string {
  const { id, name, catalogFile } = distributor;

  return `/**
 * Import Script: ${name}
 * 
 * Importa produtos do arquivo de catálogo ${catalogFile} para PostgreSQL
 * 
 * Uso:
 *   npx tsx scripts/import-${id}-to-db.ts
 * 
...
async function main() {
  console.log('\n🚀 GERANDO SCRIPTS DE IMPORT PARA TODOS OS DISTRIBUIDORES\n');
  console.log('═'.repeat(80));
  ...
  for (const distributor of distributors) {
    ...
    const scriptPath = join(scriptsDir, `import-${id}-to-db.ts`);

    // Fortlev já existe
    if (id === 'fortlev' && existsSync(scriptPath)) {
      console.log(`⏭️  Pulado: import-${id}-to-db.ts (já existe)`);
      skipped++;
      continue;
    }

    const script = generateImportScript(distributor);
    writeFileSync(scriptPath, script);

    console.log(`✅ Criado: import-${id}-to-db.ts (${script.length} bytes)`);
    created++;
  }

  console.log('\n' + '═'.repeat(80));
  console.log(`\n✨ Scripts Gerados: ${created}/${distributors.length}`);
  console.log(`⏭️  Já existentes: ${skipped}/${distributors.length}\n`);

  console.log('📋 PRÓXIMAS ETAPAS:\n');
  console.log('1️⃣  Extrair catálogos (precisa de credenciais):');
  distributors.forEach((d) => {
    if (d.id !== 'fortlev') {
      console.log(
        `   EMAIL=user PASSWORD=pass npx tsx mcp-servers/distributors/${d.id}/extract-${d.id}-full.ts`
      );
    }
  });

  console.log('\n2️⃣  Importar para PostgreSQL (após extração):');
  distributors.forEach((d) => {
    console.log(`   npx tsx scripts/import-${d.id}-to-db.ts`);
  });

  console.log('\n3️⃣  Verificar dados no banco:');
  console.log('   docker exec ysh-postgres-supabase psql -U supabase_admin -d ysh_catalog -c "SELECT distributor_id, COUNT(*) FROM ysh_catalog.products GROUP BY distributor_id"');
  console.log('\n');
}
