...
/**
 * Dynamis Custom Extractor
 * 
 * Handles custom SPA authentication flow
...
const OUTPUT_DIR = path.join(process.cwd(), 'output', 'dynamis');
...
async function loginToDynamis(page: Page, email: string, password: string): Promise<boolean> {
  try {
    ...
    console.log(`\n  📊 Indicadores positivos: ${loginResult.positiveCount}/8`);
    ...
    console.log('\n⚠️  Login incerto, aguardando verificação manual (30s)...');
  ...
  const products = await page.evaluate(() => {
    ...
    productElements.forEach((element, index) => {
      try {
        ...
        const sku = skuMatch ? skuMatch[0] : `dynamis-${index}`;
...
async function extractDynamis(): Promise<void> {
  console.log(`
╔════════════════════════════════════════════════════════════╗
║  ⚡ DYNAMIS - CUSTOM SPA AUTHENTICATION                  ║
╚════════════════════════════════════════════════════════════╝
  `);

  const email = process.env.DYNAMIS_EMAIL || '';
  const password = process.env.DYNAMIS_PASSWORD || '';

  if (!email || !password) {
    console.log('❌ Erro: DYNAMIS_EMAIL e DYNAMIS_PASSWORD devem estar definidos');
    process.exit(1);
  }

  console.log(`👤 Email: ${email}`);
  console.log(`🌐 URL: https://app.dynamisimportadora.com.br/`);
  ...
  try {
    ...
    await page.goto('https://app.dynamisimportadora.com.br/', { 
      waitUntil: 'domcontentloaded',
      timeout: 30000 
    });

    // Login
    const loginSuccess = await loginToDynamis(page, email, password);
...
// Run
extractDynamis().catch(console.error);
